...
class ServerController extends Controller
{
    public function beginLogin(Request $request)
    {
        ...
        if($request->has('token')) {
            Log::debug("ServerAuth - Setting server_client_token token for Server-Auth from token");
            ...
            Log::debug("ServerAuth - server_client_token token for Server-Auth already exists - continuing");
            ...
            Log::debug("ServerAuth - No server_client_token in session and no token in request - Aborting");
            abort(400, 'Invalid request.');
        }

        if (!Auth::check()) {
            Log::debug("ServerAuth - User not logged in - Redirecting to login page");
            return redirect('login');
        }

        $query = New ServerQuery;
        try {
            Log::debug("ServerAuth - Fetching Client IP from Game-Server");
            ...
            Log::error("ServerAuth - Error while fetching client IP from Game-Server: ".$e);
            abort(500, $e->getMessage());
        }
        if ($query->response->statuscode != '200') {
            Log::error("ServerAuth - Invalid Status-Code while fetching client IP from Game-Server: ".$query->response->statuscode);
            abort($query->response->statuscode);
        }

        if($request->getClientIp() !== $query->response->data) {
            Log::debug("ServerAuth - Player IP on Server does not match Request IP - Warning User");
            return redirect()->route('server.login.warn');
        }

        Log::debug("ServerAuth - Redirecting to next page");
    ...
    public function endLogin(Request $request)
    {
        if(!$request->session()->has('server_client_token') || !Auth::check()) {
            Log::debug("ServerAuth - Auth request does not have a server_client_token or User is not logged in - Aborting");
        ...
        if($request->user()->byond_key == null) {
            Log::debug("ServerAuth - Unable to Auth - User has no ckey linked");
            return view('auth.server.nokey');
        }
        $query = New ServerQuery;
        try {
            Log::debug("ServerAuth - Sending auth_client request to server for ckey: ".$request->user()->byond_key);
            $query->setUp(config('aurora.gameserver_address'),config('aurora.gameserver_port'),config('aurora.gameserver_auth'));
            $query->runQuery([
                'query' => 'auth_client',
                'clienttoken' => $client_token,
            ...
            Log::debug("ServerAuth - Error while sending auth_client request to server: ".$e->getMessage());
            abort(500, $e->getMessage());
        }

        if ($query->response->statuscode == '200') {
            Log::debug("ServerAuth - Ckey Succesfully logged in: ".$request->user()->byond_key);
